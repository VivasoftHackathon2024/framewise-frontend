...
function StreamVideo() {
    ...
    const analyzeStreamMutation = useMutation({
        mutationFn: async (streamId) => {
            ...
            // Generate summary and behavior first
            setIsGeneratingSummary(true);
            const [summary, customerBehavior] = await Promise.all([
                videoApi.generateSummary(lastVideoId),
                videoApi.getAgentAnalysis(lastVideoId, 'customer_behaviour')
            ]);
            
            // Update with initial results
            setAnalysisResults({
                summary_result: summary || null,
                customer_behaviour: customerBehavior || null
            });
            ...
            const [suspicious, tamper] = await Promise.allSettled([
                videoApi.getAgentAnalysis(lastVideoId, 'suspicious'),
                videoApi.getAgentAnalysis(lastVideoId, 'tamper')
            ]);
                ...
                <div className="mt-8">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        <div className="h-[600px] overflow-auto rounded-lg space-y-4">
                            ...
                            {analysisResults?.customer_behaviour && (
                                <VideoSummary 
                                    title="Customer Behavior Analysis"
                                    summaryResult={analysisResults.customer_behaviour}
                                />
                            )}
                        </div>
                    </div>
                    ...
                    <div className="space-y-4 mt-4">
                        ...
                        {analysisResults?.suspicious_evaluation && (
                            <SuspiciousAgentComponent 
                                suspiciousEvaluation={analysisResults.suspicious_evaluation} 
                            />
                        )}
                        {analysisResults?.tamper_evaluation && (
                            <TamperAgentComponent 
                                tamperEvaluation={analysisResults.tamper_evaluation} 
                            />
                        )}
                    </div>
                </div>
