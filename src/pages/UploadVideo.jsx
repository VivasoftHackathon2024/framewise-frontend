...
const UploadVideo = () => {
    ...
    const analysisMutation = useMutation({
        mutationFn: async (videoId) => {
            ...
            const summary = await videoApi.generateSummary(videoId);
            setUploadedVideo(prev => ({
                ...prev,
            ...
            const [fire, assault, crime, drug, theft] = await Promise.allSettled([
                videoApi.getAgentAnalysis(videoId, 'fire'),
                videoApi.getAgentAnalysis(videoId, 'assault'),
                videoApi.getAgentAnalysis(videoId, 'crime'),
                videoApi.getAgentAnalysis(videoId, 'drug'),
                videoApi.getAgentAnalysis(videoId, 'theft')
            ]);
                        ...
                        {uploadedVideo.analysis_result && (
                            <>
                                ...
                                <div className="space-y-4">
                                    ...
                                    {uploadedVideo.agents?.fire && (
                                        <FireAgentComponent fireEvaluation={uploadedVideo.agents.fire} />
                                    )}
                                    {uploadedVideo.agents?.assault && (
                                        <AssaultAgentComponent assaultEvaluation={uploadedVideo.agents.assault} />
                                    )}
                                    {uploadedVideo.agents?.crime && (
                                        <CrimeAgentComponent crimeEvaluation={uploadedVideo.agents.crime} />
                                    )}
                                    {uploadedVideo.agents?.drug && (
                                        <DrugAgentComponent drugEvaluation={uploadedVideo.agents.drug} />
                                    )}
                                    {uploadedVideo.agents?.theft && (
                                        <TheftAgentComponent theftEvaluation={uploadedVideo.agents.theft} />
                                    )}
                                </div>
                            </>
                        )}
